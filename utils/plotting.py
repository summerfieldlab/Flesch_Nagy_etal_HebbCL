...
def scatter_mds_2(xyz, task_id='a', fig_id=1, flipdims=False, items_per_dim=5, flipcols=False, marker_scale=1):
    ...
    _, scat_l = helper_make_colormap(basecols=np.array([[63, 39, 24], [64, 82, 21], [
                                     65, 125, 18], [66, 168, 15], [68, 255, 10]])/255, n_items=items_per_dim, monitor=False)

    b, l = np.meshgrid(np.arange(0, items_per_dim),
                       np.arange(0, items_per_dim))
    if flipdims == True:
        l, b = np.meshgrid(np.arange(0, items_per_dim),
                           np.arange(0, items_per_dim))
    ...
    if task_id == 'both':
        ...
        for ii in range(0, n_items//2):
            plt.plot([x[ii], x[ii]], [y[ii], y[ii]], marker='s', markerfacecolor=ctxMarkerCol,
                     markeredgecolor=ctxMarkerEdgeCol[0], markersize=ctxMarkerSize, markeredgewidth=.5)
            plt.plot(x[ii], y[ii], marker='h', markeredgecolor=scat_l[l[ii], :],
                     markerfacecolor=scat_l[l[ii], :], markersize=scat_b[b[ii]])

        for ii in range(n_items//2, n_items):
            plt.plot(x[ii], y[ii], marker='s', markerfacecolor=ctxMarkerCol,
                     markeredgecolor=ctxMarkerEdgeCol[1], markersize=ctxMarkerSize, markeredgewidth=.5)
            plt.plot(x[ii], y[ii], marker='h', markeredgecolor=scat_l[l[ii], :],
                     markerfacecolor=scat_l[l[ii], :], markersize=scat_b[b[ii]])
    else:
        for ii in range(0, n_items):
            ...
            plt.plot(x[ii], y[ii], marker='s', markerfacecolor=ctxMarkerCol,
                     markeredgecolor=ctxMarkerEdgeCol, markersize=ctxMarkerSize, markeredgewidth=.5)
            plt.plot(x[ii], y[ii], marker='h', markeredgecolor=scat_l[l[ii], :],
                     markerfacecolor=scat_l[l[ii], :], markersize=scat_b[b[ii]])
...
def plot_MDS_embeddings_2D(embedding, fig, fig_id=2, axlims=2.5, flipdims=False, monk=False, flipcols=False):
    ...
    plt.subplot(1, 2, 1)
    plot_grid2(embedding[0:ii_half, [0, 1]], line_width=.5,
               line_colour=col1, fig_id=fig_id, n_items=n_items)
    scatter_mds_2(embedding[0:ii_half, [0, 1]], fig_id=fig_id, task_id='a',
                  flipdims=flipdims, items_per_dim=n_items, flipcols=flipcols)
    plot_grid2(embedding[ii_half:, [0, 1]], line_width=.5,
               line_colour=col2, fig_id=fig_id, n_items=n_items)
    scatter_mds_2(embedding[ii_half:, [0, 1]], fig_id=fig_id, task_id='b',
                  flipdims=flipdims, items_per_dim=n_items, flipcols=flipcols)
    ax = plt.gca()
    ...
    plt.subplot(1, 2, 2)
    plot_grid2(embedding[0:ii_half, [1, 2]], line_width=.5,
               line_colour=col1, fig_id=fig_id, n_items=n_items)
    scatter_mds_2(embedding[0:ii_half, [1, 2]], fig_id=fig_id, task_id='a',
                  flipdims=flipdims, items_per_dim=n_items, flipcols=flipcols)
    plot_grid2(embedding[ii_half:, [1, 2]], line_width=.5,
               line_colour=col2, fig_id=fig_id, n_items=n_items)
    scatter_mds_2(embedding[ii_half:, [1, 2]], fig_id=fig_id, task_id='b',
                  flipdims=flipdims, items_per_dim=n_items, flipcols=flipcols)
    ax = plt.gca()
    ax.set_xlim([-axlims, axlims])
    ax.set_ylim([-axlims, axlims])
...
def plot_results(ws, delta_ws, n_trials, eta, sigma):
    ...
    plt.suptitle(r'$\eta = $' + f'{eta}' + r'  $\sigma_{init} = $' +
                 f'{sigma}', fontweight='normal', fontsize=18)
    plt.tight_layout()
...
def plot_initsign_results(ws, delta_ws, n_trials, eta, init_weights=[1, -1]):
    ...
    plt.ylabel('weight')
    plt.plot([n_trials//2, n_trials//2], plt.ylim(), 'k:')
    plt.suptitle(r'$\eta = $' + f'{eta}' + r'  $w_{init} = $' +
                 f'{init_weights}', fontweight='normal', fontsize=16)
    plt.tight_layout()
...
def plot_ghasla_results(data, ws, delta_ws, n_trials, eta, sigma):
    ...
    plt.yticks(ticks=np.arange(0, 1.1, 0.1), labels=(
        np.arange(0, 1.1, 0.1)*100).astype('int'))
    plt.xlabel('iter')
    plt.ylabel('percentage')
    plt.legend(['task a', 'task b'], frameon=False)
    plt.suptitle('Learning Dynamics, ' + r'$\eta = $' +
                 f'{eta}' + r'  $\sigma_{init} = $' + f'{sigma}', fontweight='normal', fontsize=12)
    plt.tight_layout()
