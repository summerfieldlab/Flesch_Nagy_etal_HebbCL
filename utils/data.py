...
def make_dataset_fromchoices(args, choices_a, choices_b):
    ...
    x_task_a, y_task_a, f_task_a = mk_block_humanchoices(
        "task_a", 0, choices_a, choices_b, args.ctx_scaling
    )
    y_task_a = y_task_a[:, np.newaxis]
    l_task_a = (y_task_a > 0).astype("int")

    x_task_b, y_task_b, f_task_b = mk_block_humanchoices(
        "task_b", 0, choices_a, choices_b, args.ctx_scaling
    )
    y_task_b = y_task_b[:, np.newaxis]
    l_task_b = (y_task_b > 0).astype("int")
