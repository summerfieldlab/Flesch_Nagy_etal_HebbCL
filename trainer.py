...
class Optimiser():
    ...
    def sla_update(self, model, x_in):
        ...
        with torch.no_grad():
            ...
            Y = Y.reshape(-1)
            x_in = x_in.reshape(-1)
            model.W_h += torch.t((torch.outer(Y, x_in) - torch.outer(Y,
                                 model.W_h @ Y) / self.hebb_normaliser) * self.lrate_hebb)
            model.zero_grad()
    ...
    def gha_update(self, model, x_in):
        ...
        with torch.no_grad():
            Y = torch.t(model.W_h) @ x_in
            model.W_h += torch.t((torch.outer(Y, x_in) - (torch.tril(torch.outer(
                Y, Y)) @ torch.t(model.W_h)) / self.hebb_normaliser) * self.lrate_hebb)
            model.zero_grad()
...
def train_model(args, model, optim, data, logger):
    ...
    x_both = torch.from_numpy(np.concatenate(
        (data['x_task_a'], data['x_task_b']), axis=0)).float().to(args.device)
    r_both = torch.from_numpy(np.concatenate(
        (data['y_task_a'], data['y_task_b']), axis=0)).float().to(args.device)
    ...
    for ii, x, y in zip(idces, x_train, y_train):
        ...
        if ii % args.log_interval == 0:
            ...
            if args.verbose:
                print('step {}, loss: task a {:.4f}, task b {:.4f} | acc: task a {:.4f}, task b {:.4f}'.format(
                    str(ii), logger.losses_1st[-1], logger.losses_2nd[-1], logger.acc_1st[-1], logger.acc_2nd[-1]))
                print('... n_a: {:d} n_b: {:d}'.format(
